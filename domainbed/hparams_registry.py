...
def _hparams(algorithm, dataset, random_seed, args=None, larger_batch=False):
    """
    ...
    _hparam('pretrained', True, lambda r: True)  # for imagenet-pretrain model

    _hparam('freeze_bn', True, lambda r: True)  # for swad bn configuration
    ...
    _hparam('clip_grad', False, lambda r: False)
    ...
    elif "MMD" in algorithm or "CORAL" in algorithm or algorithm == "CausIRL_CORAL" or algorithm == "CausIRL_MMD":
    ...
    elif algorithm in ["CAD", "CondCAD", "CoCo_CondCAD"]:
        _hparam('lmbda', 1e-5, lambda r: 1e-5)
    ...
    if "CoCo" in algorithm:
        if "SelfReg" in algorithm:
            _hparam('lmbda', 0.3, lambda r: 0.3)
        if 'cluster_num' not in hparams:
            _hparam('cluster_num', 1, lambda r: r.choice([1, 5, 10, 20, 30]))
        if 'cluster_size' not in hparams:
            _hparam('cluster_size', 30, lambda r: r.choice([10, 20, 30, 50, 100]))
        if 'quantile' not in hparams:
            _hparam('quantile', 0.01, lambda r: r.choice([0.01, 0.05, 0.1, 0.3]))
        if 'act_ratio' not in hparams:
            _hparam('act_ratio', 0.3, lambda r: r.choice([0.01, 0.05, 0.1, 0.3]))

        ###### param for clustering samples
        _hparam('ratio', 0.1, lambda r: 0.1)
        _hparam('min_num', 200, lambda r: 200)

        _hparam('use_c_weights', True, lambda r: True)
        _hparam('c_thresh', 10000, lambda r: 10000)
        _hparam('include_unact', False, lambda r: False)


    # Dataset-and-algorithm-specific hparam definitions. Each block of code
    # below corresponds to exactly one hparam. Avoid nested conditionals.

    if dataset in ['Corrupted_CIFAR_10', 'Corrupted_CIFAR_100']:
        _hparam('lr', 1e-1, lambda r: 10**r.uniform(-2, -0.5))
        _hparam('batch_size', 256, lambda r: 512)
...
def default_hparams(algorithm, dataset, args=None, larger_batch=False):
    return {a: b for a, (b, c) in _hparams(algorithm, dataset, 0, args, larger_batch).items()}


def random_hparams(algorithm, dataset, seed, args=None, larger_batch=False):
