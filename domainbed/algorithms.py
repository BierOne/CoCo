...
class ERM(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(ERM, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
        self.cifar = True if input_shape[1:] == (32, 32) else False
        ...
        if not self.cifar:
            ...
            self.optimizer = torch.optim.SGD(
                ...
                weight_decay=self.hparams['weight_decay'],
                nesterov=True)
            self.scheduler = lr_scheduler.CosineAnnealingLR(self.optimizer, hparams['steps'])
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        loss.backward()
        # print(self.featurizer.network.conv1.weight.grad)
        self.optimizer.step()
        if self.cifar:
            self.scheduler.step()
    ...
    def abstract_concept_from_feat(self, feat, all_y=None, l_name='layer4'):
        b = feat.shape[0]
        if feat.dim() > 2:
            feat = feat.view(*feat.shape[:2], -1)
            feat = feat.mean(dim=-1)  # max pooling for conv map
        # Compute concept-level contrastive loss
        # [batch, num_cluster, c_neurons]
        cluster_n_ids = self.c_clusters[l_name].cuda().expand(b, -1, -1)

        # Add padding neuron to the layer
        padding = torch.zeros((b, 1), requires_grad=False, device='cuda')  # [batch, 1]
        ...
        expanded_feat = feat.unsqueeze(1).expand(-1, num_concepts, -1)
        concept_state = expanded_feat.gather(-1, index=cluster_n_ids)

        if self.c_weights is None:
            concept_state_vec = concept_state.amax(-1)
            # concept_state_vec = (concept_state).sum(-1)
        else:
            c_weights = self.c_weights[l_name].cuda().expand(b, -1, -1)
            # concept_state_vec = (c_weights * concept_state).amax(-1)  # [b, n]
            concept_state_vec = (c_weights * concept_state).sum(-1)

        # print(all_y[:3])
        # print(cluster_n_ids.gather(2, concept_state.argmax(-1, keepdim=True))[:3, :5, 0])
        # print(concept_state_vec[:3, :5])
...
class CoCo_SelfReg(ERM):
    def __init__(self, input_shape, num_classes, num_domains, hparams, ablation=False):
        super(CoCo_SelfReg, self).__init__(input_shape, num_classes, num_domains,
                                          hparams)
        ...
        if hparams['cluster_num'] > 0:
            if hparams['include_unact']:
                self.c_size = {'layer4': hparams['cluster_num'] + hidden_size}
            else:
                self.c_size = {'layer4': hparams['cluster_num']}

            cav_size = self.c_size['layer4']
            self.abl_mlp = nn.Sequential(
                nn.Linear(input_feat_size, hidden_size),
                nn.BatchNorm1d(hidden_size),
                nn.ReLU(inplace=True),
                nn.Linear(hidden_size, cav_size),
                nn.BatchNorm1d(cav_size),
            )

            self.cdpl_cpt = nn.Sequential(
                nn.Linear(self.c_size['layer4'], hidden_size),
                nn.BatchNorm1d(hidden_size),
                nn.ReLU(inplace=True),
                nn.Linear(hidden_size, hidden_size),
                nn.BatchNorm1d(hidden_size),
                nn.ReLU(inplace=True),
                nn.Linear(hidden_size, self.c_size['layer4']),
                nn.BatchNorm1d(self.c_size['layer4'])
            )
            # self.set_grouped_optim()

        self.clip_w = 0.5 if self.hparams['resnet18'] else 0.2
        self.lmbda = self.hparams['lmbda']
        self.clip_grad = self.hparams['clip_grad']
        self.is_ablation = ablation

    def update_cl(self, minibatches, unlabeled=None):
        all_x = torch.cat([x for x, y in minibatches])
        ...
        if (not self.is_ablation) and (self.c_clusters is not None):
            feat = self.abstract_concept_from_feat(feat, all_y)
            proj = self.cdpl_cpt(feat)
        else:
            proj = self.cdpl(feat)
        ...
        L_ind_feat = self.lmbda * self.MSEloss(feat, feat_2)
        L_hdl_feat = self.lmbda * self.MSEloss(feat, feat_3)
        ...
        loss.backward()
        if self.clip_grad:
            nn.utils.clip_grad_norm_(self.network.parameters(), self.clip_w)
        self.optimizer.step()
    ...
    def extract_feat(self, x, return_cav=False):
        feat = self.featurizer(x)
        output = self.classifier(feat)
        if return_cav:
            cav = None
            if (not self.is_ablation) and (self.c_clusters is not None):
                cav = self.abstract_concept_from_feat(feat)
            return output, feat, cav
        return output, feat
...
class OursAbstractCAD(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains,
                 hparams, is_conditional, is_ablation=False):
        super(OursAbstractCAD, self).__init__(input_shape, num_classes, num_domains, hparams)
        ...
        self.is_normalized = hparams[
            'is_normalized']  # whether apply normalization to representation when computing loss
        ...
        self.unact_neurons = None
        if hparams['include_unact']:
            self.c_size = {'layer4': hparams['cluster_num'] + self.featurizer.n_outputs}
        else:
            self.c_size = {'layer4': hparams['cluster_num']}
        self.clip_w = 0.2
        ...
        input_feat_size = self.featurizer.n_outputs
        self.abl_mlp = nn.Sequential(
            nn.Linear(input_feat_size, 512),
            nn.BatchNorm1d(512),
            nn.ReLU(inplace=True),
            nn.Linear(512, cav_size),
            nn.BatchNorm1d(cav_size),
        )

    def set_grouped_optim(self, target_layer="network.layer4", lr=5e-6):
        grouped_parameters = [{'params': [p for n, p in self.network.named_parameters() if target_layer not in n],
                               'lr': self.hparams["lr"], 'weight_decay': self.hparams['weight_decay']},
                              {'params': [p for n, p in self.network.named_parameters() if target_layer in n],
                               'lr': self.hparams["lr"], 'weight_decay': self.hparams['weight_decay']}]
        self.optimizer = torch.optim.Adam(grouped_parameters)
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        return {'loss': loss.item(), 'bn_loss': 0}
    ...
    def update_cl(self, minibatches, unlabeled=None):
        ...
        if (not self.is_ablation) and (self.c_clusters is not None):
            all_z = self.abstract_concept_from_feat(all_z)
        ...
        cl_time = time.time() - step_start_time

        self.optimizer.zero_grad()
        total_loss.backward()
        # nn.utils.clip_grad_norm_(self.parameters(), self.clip_w)
        self.optimizer.step()
    ...
    def abstract_concept_from_feat(self, feat, l_name='layer4'):
        b = feat.shape[0]
        if feat.dim() > 2:
            feat = feat.view(*feat.shape[:2], -1)
            feat = feat.mean(dim=-1)  # max pooling for conv map
...
class CoCo_CondCAD(OursAbstractCAD):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(CoCo_CondCAD, self).__init__(input_shape, num_classes, num_domains, hparams, is_conditional=True)
...
class AbstractCAD(Algorithm):
    ...
    def bn_loss(self, z, y, dom_labels):
        ...
        mask_y = torch.eq(y, y.T).to(device)
        mask_d = (torch.eq(dom_labels, dom_labels.T)).to(device)
        mask_drop = ~torch.eye(batch_size, device=device).bool()  # drop the "current"/"self" example
        mask_y &= mask_drop
        mask_y_n_d = mask_y & (~mask_d)  # contain the same label but from different domains
        mask_y_d = mask_y & mask_d  # contain the same label and the same domain
        mask_y, mask_drop, mask_y_n_d, mask_y_d = mask_y.float(), mask_drop.float(), mask_y_n_d.float(), mask_y_d.float()
        ...
        def finite_mean(x):
            # only 1D for now
            num_finite = (torch.isfinite(x).cuda().float()).sum()
            mean = torch.where(torch.isfinite(x), x, torch.tensor(0.0).to(x)).sum()
            if num_finite != 0:
                mean = mean / num_finite
            else:
...
class Fish(Algorithm):
    ...
    def extract_feat(self, x):
        return self.network.extract_feat(x)
...
class OursAbstractMMD(ERM):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams, gaussian):
        super(OursAbstractMMD, self).__init__(input_shape, num_classes, num_domains,
                                          hparams)
        if gaussian:
            self.kernel_type = "gaussian"
        else:
    ...
    def update_cl(self, minibatches, unlabeled=None):
        ...
        if (not self.is_ablation) and (self.c_clusters is not None):
            features = [self.abstract_concept_from_feat(b_feat) for b_feat in features]
...
class OursMMD(OursAbstractMMD):
    """
    MMD using Gaussian kernel
    """

    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(OursMMD, self).__init__(input_shape, num_classes,
                                  num_domains, hparams, gaussian=True)


class OursCORAL(OursAbstractMMD):
    """
    MMD using mean and covariance difference
    """

    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(OursCORAL, self).__init__(input_shape, num_classes,
                                    num_domains, hparams, gaussian=False)
...
class MTL(Algorithm):
    ...
    def extract_feat(self, x, env=None):
        feat = self.featurizer(x)
        embedding = self.update_embeddings_(feat, env).normal_()
        return self.classifier(torch.cat((feat, embedding), 1)), feat
...
class SagNet(Algorithm):
    ...
    def extract_feat(self, x):
        feat = self.network_f(x)
        return self.network_c(feat), feat
...
class IB_IRM(ERM):
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        if self.update_count == self.hparams['irm_penalty_anneal_iters'] or self.update_count == self.hparams[
