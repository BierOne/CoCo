...
# Filtered_Dataset_Names = ['RotatedMNIST', 'ColoredMNIST']  # for latex print
...
def print_results_tables(records, selection_method, latex):
    ...
    #     trial_accs = (grouped_records
    #         .filter_equals(
    #             "dataset, algorithm, trial_seed",
    #             (dataset, algorithm, trial_seed)
    #         ).select("sweep_acc"))
    #     if len(trial_accs) >0:
    #         print(f"trial_seed:{trial_seed}", sum(trial_accs) / len(trial_accs))
...
if __name__ == "__main__":
    ...
    records = reporting.load_records(args.input_dir, args.recurs)
