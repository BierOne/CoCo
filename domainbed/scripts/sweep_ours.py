...
class Job:
    ...
    def __init__(self, train_args, sweep_output_dir):
        ...
        command = ['python', '-m', 'domainbed.scripts.train_ours_coverage']
...
def make_args_list(n_trials, dataset_names, algorithms, n_hparams_from, n_hparams, steps,
                data_dir, task, holdout_fraction, single_test_envs, resnet18, multi_level,
                n_trials_from, hparams, specified_test):
    args_list = []
    for trial_seed in range(n_trials_from, n_trials):
        for dataset in dataset_names:
            for algorithm in algorithms:
                if specified_test is not None:
                    all_test_envs = [[specified_test]]
                else:
                    if single_test_envs:
                        all_test_envs = [
...
if __name__ == "__main__":
    ...
    parser.add_argument('command', choices=['launch', 'delete', 'delete_incomplete', 'delete_and_launch'])
    ...
    parser.add_argument('--multi_level', type=bool, default=True, help="hie clustering")
    parser.add_argument('--specified_test', type=int, default=None)
    ...
    args_list = make_args_list(
        ...
        specified_test=args.specified_test
    )
    ...
    if args.command == 'delete_and_launch':
